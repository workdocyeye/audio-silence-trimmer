...
克隆本仓库到 OpenCode 的 skills 目录：

```bash
git clone https://github.com/workdocyeye/audio-silence-trimmer.git ~/.config/opencode/skills/audio-silence-trimmer
```

安装后重启 OpenCode，在对话中 使用 `/audio-silence-trimmer` 命令并`@` 引用你的音频和 SRT 文件，即可调用。

### 方式二：直接运行 Python

```bash
git clone https://github.com/workdocyeye/audio-silence-trimmer.git
cd audio-silence-trimmer
pip install -r requirements.txt  # 如果有
python scripts/trim_audio.py <input.mp3> <input.srt>
```
